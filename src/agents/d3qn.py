...
def train(level: int, wall_obstacles: bool, episodes: int, seed: int = None, trial=None, config_file: str = None, render: bool = False, prefix: str = None):
    ...
    base_name = f"{prefix}" if prefix else f"d3qn_level{level}{'_wall' if wall_obstacles else ''}"
...
def _load_once():
    ...
    base_name = f"{_CURRENT_PREFIX}" if _CURRENT_PREFIX else f"d3qn_level{_CURRENT_LEVEL}{'_wall' if _CURRENT_WALL else ''}"
