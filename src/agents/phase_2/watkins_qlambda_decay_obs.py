...
- State Augmentation: 18-bit sensor + Temporal Decay Bins (0, 1-10, 11-20, >20) mapping to 1,048,576 integer states.
...
def train(level: int, wall_obstacles: bool, episodes: int, config_file: str = None, render: bool = False, prefix: str = None, trial=None):
    ...
    print("Training watkins_qlambda_decay_obs for level", level, "with wall obstacles", wall_obstacles, "for", episodes, "episodes")
    ...
    for episode in range(episodes):
        ...
        time_since_seen = 100
        time_since_seen = 0 if np.any(obs[:17] > 0) else time_since_seen + 1
        decay_bin = 0 if time_since_seen == 0 else (1 if time_since_seen <= 10 else (2 if time_since_seen <= 20 else 3))
        ...
        for step in range(config["max_steps"]):
            ...
            time_since_seen_next = 0 if np.any(next_obs[:17] > 0) else time_since_seen + 1
            decay_bin_next = 0 if time_since_seen_next == 0 else (1 if time_since_seen_next <= 10 else (2 if time_since_seen_next <= 20 else 3))
            next_stateID = obs_to_state(next_obs, decay_bin_next)
            ...
            obs = next_obs
            time_since_seen = time_since_seen_next
    ...
    base_name = f"{prefix}" if prefix else f"watkins_qlambda_decay_obs_level{level}{'_wall' if wall_obstacles else ''}"
...
def _load_once():
    global _Q_TABLE
    if _Q_TABLE is None:
        base_name = f"{_CURRENT_PREFIX}" if _CURRENT_PREFIX else f"watkins_qlambda_decay_obs_level{_CURRENT_LEVEL}{'_wall' if _CURRENT_WALL else ''}"
