...
def train(level: int, wall_obstacles: bool, episodes: int, config_file: str = None, render: bool = False, prefix: str = None, trial=None):
    ...
    print("Training exp_sarsa_prev_action for level", level, "with wall obstacles", wall_obstacles, "for", episodes, "episodes")
...
def _load_once():
    global _Q_TABLE
    if _Q_TABLE is None:
        base_name = f"{_CURRENT_PREFIX}" if _CURRENT_PREFIX else f"exp_sarsa_prev_action_level{_CURRENT_LEVEL}{'_wall' if _CURRENT_WALL else ''}"
