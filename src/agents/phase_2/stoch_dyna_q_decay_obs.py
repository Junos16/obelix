...
_Q_TABLE = None
STATE_SPACE_SIZE = (2**18) * 4  # 18 bits + 4 Temporal Decay Bins
...
def train(level: int, wall_obstacles: bool, episodes: int, config_file: str = None, render: bool = False, prefix: str = None, trial=None):
    ...
    for episode in range(episodes):
        ...
        time_since_seen = 100 # Default to "Completely Gone"
        ...
        for _ in range(config["max_steps"]):
            ...
            next_tss = 0 if np.any(next_obs[:17] > 0) else time_since_seen + 1
            next_bin = 0 if next_tss == 0 else (1 if next_tss <= 10 else (2 if next_tss <= 20 else 3))
            next_stateID = obs_to_state(next_obs, next_bin)
